...
export async function POST(request: NextRequest) {
  ...
  if (source === 'nasta') {
    try {
      ...
      await writeSyncLog({ created: 0, updated: 0, removed: 0, totalScraped: 0, errors: 0, durationMs: 0 }, 'failed', message)
  ...
  if (source === 'hesselberg') {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      await writeHesselbergSyncLog({ created: 0, updated: 0, removed: 0, totalScraped: 0, errors: 0, durationMs: 0 }, 'failed', message)
      return NextResponse.json({ error: message }, { status: 500 })
    }
  }
